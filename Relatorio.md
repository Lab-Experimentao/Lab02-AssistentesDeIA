...
O experimento respondeu com clareza apenas uma das três RQs do enunciado. **RQ1 (tempo) é confirmada**, com o maior efeito observado em todo o estudo: o uso do assistente de IA reduziu de forma consistente e estatisticamente significativa o tempo até passar em todos os testes de aceitação, em todos os 6 katas, sem exceção. **RQ2 (defeitos) não pôde ser testada**: os 18 trials terminaram com 100% de sucesso nos testes, `com_ia` e `sem_ia`, um efeito-teto do próprio desenho (katas calibrados para caber no time-box nos dois tratamentos), não uma falha de coleta, mas que deixa a pergunta sem resposta estatística possível. **RQ3 (estrutura do código) fica sem resposta conclusiva**: duplicação não diferiu (0% nos dois tratamentos), e a complexidade ciclomática mostrou sinais que se invertem conforme a métrica é olhada bruta ou pareada por kata, evidência de que o contrabalanceamento (que troca o integrante, não o kata, entre tratamentos) confunde a comparação simples: a expectativa inicial de "pouca diferença" nem é confirmada nem é refutada com segurança.

Das seis frentes de inovação, a mais reveladora foi a de verbosidade e estilo: o código `com_ia` saiu mais extenso (LOC maior, com efeito grande na comparação pareada por kata), mas isso não se traduziu em pior conformidade de estilo por linha de código, e a correlação entre tempo e violações não sustentou de forma consistente a ideia de que "pressa" explique a diferença de conformidade entre tratamentos. A varredura de segurança não encontrou nenhuma vulnerabilidade em nenhum tratamento, resultado esperado dado o escopo dos katas, não uma conclusão sobre segurança de código gerado por IA em geral. As três inovações metodológicas (tamanho de efeito, tratamento de outliers e dashboard) não geraram um achado próprio, mas aumentaram a confiança nos achados de RQ1 e RQ3: sem elas, o grupo teria só um p-valor por RQ, sem saber se "não significativo" queria dizer "sem efeito" ou "amostra pequena demais para detectar".

**Limitações do estudo.** A principal é o tamanho da amostra: 3 integrantes e 18 trials dão poder estatístico suficiente para um efeito tão grande quanto o de RQ1, mas não para os efeitos pequenos/mistos de RQ3 e das inovações de estilo, onde a maioria das comparações não paramétricas fica com n=6 (Wilcoxon pareado por kata) ou 9 vs. 9 (Mann-Whitney). Das ameaças à validade levantadas no Desenho do Experimento ([HIPOTESES.md](HIPOTESES.md#ameaças-à-validade)), duas ficaram sem mitigação experimental por não haver como controlá-las com um trio: familiaridade prévia de cada integrante com o assistente de IA, e diferenças residuais de ambiente/hardware entre as três máquinas. A coleta em si cobriu um período curto (14 a 16 de setembro de 2026), o que reduz o risco de deriva do modelo de IA entre trials, mas também significa que o estudo é uma fotografia pontual, não uma tendência ao longo do tempo. Por fim, ao revisar o repositório para este relatório, o grupo identificou uma inconsistência de documentação ainda não resolvida entre `README.md` (que descreve o assistente `com_ia` como Claude via Claude Code, CLI) e `HIPOTESES.md`/`DESENHO_EXPERIMENTO.md` (que descrevem Claude via claude.ai, navegador); os dois não podem estar certos ao mesmo tempo, e o grupo precisa confirmar qual interface cada integrante de fato usou antes de considerar o tratamento `com_ia` verdadeiramente uniforme entre os 9 trials.

Com mais tempo ou uma amostra maior, o grupo priorizaria três direções. Primeiro, resolver a inconsistência acima e, se necessário, tratar a interface de IA (chat vs. agente de linha de comando) como uma variável controlada explícita, em vez de um detalhe implícito do tratamento `com_ia`; as duas formas de uso podem ter efeitos bem diferentes sobre tempo e qualidade. Segundo, ampliar o número de integrantes ou de katas por tratamento, para dar à RQ3 e às inovações de estilo o mesmo poder estatístico que RQ1 já tem. Terceiro, desenhar katas com superfície de ataque real (I/O, SQL, serialização) num próximo experimento, para que a varredura de segurança deixe de ser um teste degenerado por falta de variância e possa de fato comparar `com_ia` e `sem_ia` nessa dimensão. Das inovações propostas, a correlação tempo × violações e o tamanho de efeito são as que mais valeriam a pena expandir: a primeira, com mais trials, poderia confirmar ou descartar de vez a hipótese de trade-off velocidade/qualidade específico da IA; a segunda se tornaria ainda mais informativa numa amostra onde RQ3 também tivesse poder estatístico suficiente para uma leitura conclusiva.
